...
ind = cellstrfnd(featurenames, 'Blue') &...
      cellstrfnd(featurenames, '_IntegratedIntensity') &...
...
ind = cellstrfnd(featurenames, 'Blue') &...
      ...
      cellstrfnd(featurenames, '_Variance');
dapi_text1 = data(:, ind);

ind = cellstrfnd(featurenames, 'Blue') &...
      cellstrfnd(featurenames, 'Texture') &...
      cellstrfnd(featurenames, 'Nuclei') &...
      cellstrfnd(featurenames, '_SumVariance');
dapi_text2 = data(:, ind);
...
G1 = find(G1);
G1 = G1(~debris(cyclerfeatures(G1, 1:2), 2));

S = find(S);
S = S(~debris(cyclerfeatures(S, [1 2 5]), 2));

G2 = find(G2);
G2 = G2(~debris(cyclerfeatures(G2, 1:2), 1));
