function myplotclr(x,y,z,v,marker, map, clim, draw_grid)
...
if (miv>=min(v) || mav<max(v))
    iv = v<=miv;
    plot3(x(iv),y(iv),z(iv),marker,'color',map(1,:),'markerfacecolor',map(1,:))
    hold on;
    iv = v>mav; 
    plot3(x(iv),y(iv),z(iv),marker,'color',map(end,:),'markerfacecolor',map(end,:))
end
